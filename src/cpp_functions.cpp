...
double hsic_cpp(NumericMatrix x, NumericMatrix y, std::string type = "euclidean",
                ...
                Nullable<IntegerVector> group_ = R_NilValue, bool u_center = false,
                bool is_distance = false) {
  // Get environment
  Environment env = Environment::global_env();
  Function KDist_matrix = env["KDist_matrix"];
  ...
  if (is_distance) {
    ...
    // Calculate distance matrices using KDist_matrix
    dist_x_rcpp = KDist_matrix(x, type, bw_sexp, expo, scale_factor, group_);
    if (same_matrices) {
      dist_y_rcpp = dist_x_rcpp; // Reuse the same distance matrix
    } else {
      dist_y_rcpp = KDist_matrix(y, type, bw_sexp, expo, scale_factor, group_);
    }
  }
...
double jhsic_v(Rcpp::List x, double cc = 1.0, std::string type = "euclidean", 
               ...
               double scale_factor = 0.5, Nullable<IntegerVector> group_ = R_NilValue) {
  // Get environment
  Environment env = Environment::global_env();
  Function KDist_matrix = env["KDist_matrix"];
  Function matrix_v_center = env["matrix_v_center"];
  ...
  NumericMatrix D_rcpp = KDist_matrix(first_matrix, type, bw_sexp, expo, scale_factor, group_);
  ...
  for (int i = 1; i < d; i++) {
    NumericMatrix curr_matrix = x[i];
    NumericMatrix D_curr_rcpp = KDist_matrix(curr_matrix, type, bw_sexp, expo, scale_factor, group_);
...
double jhsic_u(Rcpp::List x, double cc = 1.0, std::string type = "euclidean", 
  ...
  Environment env = Environment::global_env();
  Function KDist_matrix = env["KDist_matrix"];
  Function matrix_u_center = env["matrix_u_center"];
  Function hsic = env["hsic"];
  ...
  if (scale) {
    ...
    double scale_factor_hsic = as<double>(hsic(first_matrix, first_matrix, type, bw_sexp, expo, scale_factor, group_, true));
    // Apply scaling
  ...
  for (int i = 1; i < d; i++) {
    ...
    if (scale) {
      ...
      double scale_curr = as<double>(hsic(curr_matrix, curr_matrix, type, bw_sexp, expo, scale_factor, group_, true));
      // Apply scaling
...
// High-order HSIC
// [[Rcpp::export]]
double hohsic(List x, std::string type = "euclidean", std::string stat_type = "V", 
              SEXP bw_sexp = R_NilValue, double expo = 1, 
              double scale_factor = 0.5, Nullable<IntegerVector> group_ = R_NilValue) {
  // Get environment
  Environment env = Environment::global_env();
  ...
  // Get first matrix and sample size
  NumericMatrix first_matrix = x[0];
  int n = first_matrix.nrow();
  ...
  if (stat_type == "U") {
    // U-statistic version
    NumericMatrix D_rcpp = KDist_matrix(first_matrix, type, bw_sexp, expo, scale_factor, group_);
    arma::mat A = as<arma::mat>(matrix_u_center(D_rcpp));
    
    // Process remaining matrices
    for (int i = 1; i < d; i++) {
      NumericMatrix curr_matrix = x[i];
      NumericMatrix D_curr_rcpp = KDist_matrix(curr_matrix, type, bw_sexp, expo, scale_factor, group_);
      arma::mat centered_curr = as<arma::mat>(matrix_u_center(D_curr_rcpp));
    ...
    // V-statistic version
    NumericMatrix D_rcpp = KDist_matrix(first_matrix, type, bw_sexp, expo, scale_factor, group_);
    arma::mat A = as<arma::mat>(matrix_v_center(D_rcpp));
    
    // Process remaining matrices
    for (int i = 1; i < d; i++) {
      NumericMatrix curr_matrix = x[i];
      NumericMatrix D_curr_rcpp = KDist_matrix(curr_matrix, type, bw_sexp, expo, scale_factor, group_);
      arma::mat centered_curr = as<arma::mat>(matrix_v_center(D_curr_rcpp));
    ...
    stop("Invalid stat_type: must be either U or V");
  }
}
...
double dhsic_fast_rcpp(List x_list, 
  ...
  NumericMatrix M = as<NumericMatrix>(KDist_matrix(x1, type, bw_k, expo, scale_factor, group_));
  ...
  for (int k = 1; k < d; k++) {
    ...
    // Calculate next distance matrix with proper bandwidth
    NumericMatrix D = as<NumericMatrix>(KDist_matrix(as<NumericMatrix>(x_list[k]), type, bw_k, expo, scale_factor, group_));
...
List mhsic_cpp(NumericMatrix x, std::string type = "euclidean", 
  ...
  // Get environment
  Environment env = Environment::global_env();
  Function hsic_cpp_function = env["hsic_cpp"];
